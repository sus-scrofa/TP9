...
FT_HANDLE * initLCD(int iDevice)
{
	...
	{
		if (FT_SetBitMode(*temp, SET_BIT_MODE_MASK, SET_BIT_MODE_MODE) == FT_OK)
		{
			if (lcdWriteNibble(temp, 0x03, IR) == FT_OK)
			{
				...
				if (lcdWriteNibble(temp, 0x03, IR) == FT_OK)
				{
					...
					if (lcdWriteNibble(temp, 0x03, IR) == FT_OK)
					{
						...
						{
							std::this_thread::sleep_for(std::chrono::milliseconds(1));
							if (lcdWriteByte(temp, LCD_FUNCSET | LCD_F_2LINE, IR) == FT_OK)
							{
								if (lcdWriteByte(temp,LCD_CONTROL|LCD_F_DISPLAY_ON|LCD_F_CURSOR_ON|LCD_F_BLINK_ON,IR)==FT_OK)
								{
									if (lcdWriteByte(temp, LCD_CLEAR, IR) == FT_OK)
									{
										if (lcdWriteByte(temp, LCD_ENTSET|LCD_F_INC, IR) == FT_OK)
										{
											return temp;
										}
									}
